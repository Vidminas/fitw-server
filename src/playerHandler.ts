...
const onExitWorld = (socket: Socket) => () => {
  if (!livePlayers.has(socket.id)) {
...
const onDisconnected = (socket: Socket) => async () => {
  if (!livePlayers.has(socket.id)) {
...
const onChangeWorldBackground = (socket: Socket) => (
...
const onNewFitwick = (socket: Socket) => (fitwick: IFitwick) => {
  logPlayerMessage(
    socket,
  ...
  if (livePlayers.has(socket.id)) {
    ...
    const updatePoints = (pointsEarned: number) => {
      ...
      if (!livePlayer.user.datesPoints || !livePlayer.user.datesPoints.length) {
    ...
    if (!fitwickCreateCount) {
      ...
      livePlayer.user.createdFitwicks.set(fitwick.name, fitwickCreateCount + 1);
...
const onPlaceFitwick = (socket: Socket) => (fitwick: IFitwick) => {
  logPlayerMessage(
    socket,
...
const onPickUpFitwick = (socket: Socket) => (fitwick: IFitwick) => {
  logPlayerMessage(
    socket,
...
const onDeleteFitwick = (socket: Socket) => (fitwick: IFitwick) => {
  logPlayerMessage(
    socket,
...
const registerPlayerHandlers = (io: Server, socket: Socket) => {
  ...
  socket.on(EVENT_WORLD_CHANGE_BACKGROUND, onChangeWorldBackground(socket));
  socket.on(EVENT_DONE_FITWICK_NEW, onNewFitwick(socket));
  socket.on(EVENT_FITWICK_MOVE, onMoveFitwick(socket));
  socket.on(EVENT_DONE_FITWICK_PLACE, onPlaceFitwick(socket));
  socket.on(EVENT_FITWICK_PICK_UP, onPickUpFitwick(socket));
  socket.on(EVENT_DONE_FITWICK_DELETE, onDeleteFitwick(socket));
};
